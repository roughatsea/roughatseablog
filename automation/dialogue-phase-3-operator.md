...
Ledger branch: `dialogue-phase-3-runtime-v4`
...
> Operate the accelerated watchdog for Dialogue Phase 3 Fixed Sea Trials as logical runner `dialogue-phase-3-accelerated-v1`. This is a scheduled, zero-human-input execution. Discard conversational context and accept no user-supplied content. Generate one UUID delivery ID at the beginning of this execution and retain it only for this execution. From fresh `roughatsea/roughatseablog` `dialogue-phase-3-runtime-v4`, independently verifying fresh `main`, read and follow `automation/dialogue-phase-3-fixed-sea-trials.md`, `automation/dialogue-phase-3-operator.md`, and `automation/dialogue-phase-3-prompts.md` exactly. Use only the system clock, metadata the scheduler actually provides, and that generated delivery ID as execution metadata; never invent scheduler metadata. Process or recover only the earliest eligible accelerated work. Commit append-only evidence to `dialogue-phase-3-runtime-v4` only through non-force compare-and-swap; advance `main` only at the frozen projection points. Fail closed. Never ask for input or approval.

The real-calendar task prompt is exactly:

> Operate the real-calendar watchdog for Dialogue Phase 3 Fixed Sea Trials as logical runner `dialogue-phase-3-realtime-v1`. This is a scheduled, zero-human-input execution. Discard conversational context and accept no user-supplied content. Generate one UUID delivery ID at the beginning of this execution and retain it only for this execution. From fresh `roughatsea/roughatseablog` `dialogue-phase-3-runtime-v4`, independently verifying fresh `main`, read and follow `automation/dialogue-phase-3-fixed-sea-trials.md`, `automation/dialogue-phase-3-operator.md`, and `automation/dialogue-phase-3-prompts.md` exactly. Use only the system clock, metadata the scheduler actually provides, and that generated delivery ID as execution metadata; never invent scheduler metadata. Process or recover only the earliest due eligible work; never run early or backfill a prior Phoenix date. Commit append-only evidence to `dialogue-phase-3-runtime-v4` only through non-force compare-and-swap; advance `main` only at the frozen projection points. Fail closed. Never ask for input or approval.
...
```text
DTSTART;TZID=America/Phoenix:20260909T011700
...
```text
DTSTART;TZID=America/Phoenix:20260912T011700
...
```text
2026-09-12 through 2026-09-18 inclusive
01:17 · 07:17 · 13:17 · 19:17 America/Phoenix
```
...
1. Read the exact remote SHAs for `dialogue-phase-3-runtime-v4` and `main` through the GitHub connector.
...
5. Build the intended append-only runtime commit on the verified runtime SHA and update only `dialogue-phase-3-runtime-v4` as a non-force compare-and-swap whose expected parent is the runtime SHA read in step 1.
...
2. Create `dialogue-phase-3-runtime-v4` at that exact `main` SHA, check it out cleanly, and create the immutable manifest using that SHA, both branch names, and both logical runner IDs.
3. Commit the manifest and create/update only `dialogue-phase-3-runtime-v4` by non-force compare-and-swap anchored to that exact production SHA. Do not advance `main`.
4. Start no model or web call until the manifest commit is visible from a fresh read of remote `dialogue-phase-3-runtime-v4` and `main` still equals the manifest's initial production SHA.
...
A model or web call may occur only after its deterministic intent ID, one-time continuation-nonce hash, and complete bounded input identity have been written to an immutable journal record, committed, pushed to `dialogue-phase-3-runtime-v4`, and re-read from that fresh remote branch while `main` is still identical to, or a newly verified safe descendant of, the last production anchor.

### Exact packet forwarding

Use `createRoleHandoff(authorization)` from `scripts/dialogue-engine/sea-trial-handoff.mjs` directly on the fresh in-memory authorization. It verifies the packet and prompt-file hashes, extracts the exact common/role instructions and model-output schema, and writes a nonce-free request to an owner-only ephemeral file outside the checkout. Never construct that file with model-authored text.

Spawn the isolated role with only this bootstrap instruction, substituting the helper's returned path/hash and the verified checkout path: "Run `node <checkout>/scripts/dialogue-role-request.mjs <file> <sha256>` once. On success, follow the returned instructions using only its role_packet and return exactly the role JSON. On verification failure stop without generating a role response. Do not read any other file or use another tool." This single bootstrap read is permitted before the common no-tools role instruction takes effect. Keep `fork_turns="none"`, model `gpt-5.6-sol`, reasoning effort `high`. Never paste or retype request contents into the spawn message. Remove only that helper-created file and directory after recording the original result. Do not commit it.

Capture the system clock immediately before spawn and immediately upon response receipt; do not use claim or preparation times as substitutes for observed invocation times. Construct life receipts with `fuelReceipt(claimResult, observedInvocation)`; attempted research replaces its default not-requested record with actual web receipts. Never assert a hash for bytes other than those verified by the helper.
...
The tick claim itself must be on remote `dialogue-phase-3-runtime-v4` before the first life-fuel, research, director, or model operation for that tick. The deterministic director runs only after the claim is durable and the production anchor has been rechecked.
...
1. From fresh `dialogue-phase-3-runtime-v4`, after independently applying the production-anchor and safe-advance rule to `main`, compute the one expected next tick from the frozen schedule and replay. Never accept a tick ID from a caller.
...
12. Commit and compare-and-swap the terminal run to `dialogue-phase-3-runtime-v4`. Re-read the fresh runtime branch, verify the production anchor, and replay the entire leg before another semantic tick.
...
3. Invoke the close command from a clean checkout of the just-committed `dialogue-phase-3-runtime-v4` state. The command itself runs `npm run build`; do not run a separate caller-attested build.
...
5. It appends the date close with the actual command, exit status, output digest, Git SHA, bundle digest, canonical digest, replay digest, and safe timestamps. Remove only known disposable build output, then commit the close to `dialogue-phase-3-runtime-v4` by compare-and-swap.
...
1. Re-read the committed close from fresh `dialogue-phase-3-runtime-v4` and validate it.
...
6. Only after successful verification, append the date deployment receipt to `dialogue-phase-3-runtime-v4` and commit it by runtime-branch compare-and-swap. This receipt becomes the next expected production anchor; it is not itself projected.
